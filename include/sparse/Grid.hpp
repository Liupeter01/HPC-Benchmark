...
namespace logn {
template <typename _Ty = bool> struct Grid {
...
namespace o1 {
template <typename _Ty> struct Grid {
  using Coord2D = std::pair<std::intptr_t, std::intptr_t>;
  struct Hasher {
...
namespace o1 {
template <typename _Ty, std::intptr_t BlockSize> struct BlockGrid {
  ...
  template <typename _Ty, std::intptr_t BlockSize> struct BlockSquare {
    static constexpr std::intptr_t absmod(const std::intptr_t a,
                                          const std::intptr_t divv) {
      return a & (divv - 1);
    }
...
struct PointerGrid {
  ...
  void createByMutexLock(const std::intptr_t x, const std::intptr_t y,
                         const _Ty &value) {
    auto &block =
        m_data[(x >> BlockShift) & GridMask][(y >> BlockShift) & GridMask];
    {
      std::lock_guard<std::mutex> _lck(
          m_lock[(x >> BlockShift) & GridMask][(y >> BlockShift) & GridMask]);
      if (!block)
        block = std::make_unique<BlockSquare<_Ty, BlockSize>>();
    }
    (*block)(x, y) = value;
  }
  void createByDoubleCheckLock(const std::intptr_t x, const std::intptr_t y,
                               const _Ty &value) {
    auto &block =
        m_data[(x >> BlockShift) & GridMask][(y >> BlockShift) & GridMask];
    if (!block) {
