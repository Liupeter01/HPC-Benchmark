 ...
 * @brief Definition of a high-performance, cache-aligned, padded, N-dimensional flat array.
 */
...
namespace hpc {
          ...
          /**
           * @brief A high-performance, flat N-dimensional array with padding (ghost cells) and aligned memory.
           *
           * This class stores N-dimensional data using a flat, contiguous memory buffer (row-major layout),
           * optimized for HPC and SIMD workloads. It supports boundary padding (ghost cells) on each side
           * of each dimension via the `Low_Bound` and `High_Bound` template parameters. Memory alignment
           * is guaranteed via a customizable allocator (default is `AlignedAllocator` with configurable alignment).
           *
           * @tparam Dimension     Number of dimensions (must be > 0)
           * @tparam _Ty           Value type stored in the array (e.g. float, double, int)
           * @tparam Low_Bound     Number of ghost cells (padding) on the lower side of each dimension
           * @tparam High_Bound    Number of ghost cells (padding) on the upper side of each dimension (default = Low_Bound)
           * @tparam Alignment     Memory alignment in bytes (default = 16). Should be 16/32 for SSE/AVX.
           * @tparam Alloc         Custom STL-compatible allocator (default = AlignedAllocator<_Ty, Alignment>)
           *
           * @note Indexing follows row-major order: the last dimension changes fastest.
           * @note The array does not perform bounds checking unless using `at()` or `safe_linearize`.
           * @note Indexing assumes padded bounds: valid access indices range from `-Low_Bound` to `dim[i] + High_Bound - 1`.
           *
           * @example
           * @code
           * HPCHighDimensionFlatArray<2, float, 1> array(64, 64);  // 2D array with 1 ghost cell padding
           * array(0, 0) = 1.0f;
           * float val = array.at({-1, 0});  // safe access with boundary check
           * @endcode
           */
          template<std::size_t Dimension,
                    typename _Ty,
                    std::size_t Low_Bound = 0,
                    std::size_t High_Bound = Low_Bound,
...
#if _HAS_CXX20
                    static_assert(std::is_same_v<std::remove_cvref_t<_Ty>, _Ty>, "_Ty must not be cvref");
#else
                    static_assert(std::is_same_v<std::remove_cv_t<std::remove_reference_t<_Ty>>, _Ty>, "_Ty must not be cvref");
#endif
                    ...
                    public:
                              ...
                              template<typename ... DimForEachLayer,
                                        std::enable_if_t<(sizeof...(DimForEachLayer) == Dimension &&
                                                  std::conjunction_v<std::is_integral<DimForEachLayer>...>), int> = 0>
                              explicit HPCHighDimensionFlatArray(const DimForEachLayer& ... dims)
                                        : HPCHighDimensionFlatArray(std::array<std::size_t, Dimension>{static_cast<std::size_t>(dims)...}) {
                              }
                              ...
                              template<typename... Indicies,
                                        std::enable_if_t<(sizeof...(Indicies) == Dimension &&
                                                  std::conjunction_v<std::is_integral<Indicies>...>), int> = 0 >
                    ...
                    protected:
                              /**
                               * @brief Resizes the array using a dimension array, preserving ghost cell configuration.
                               *
                               * @param dim Dimension sizes per axis.
                               * @param value Initial value to fill the flat buffer.
                               */
                              void resize(const std::array<std::size_t, Dimension>& dim, const _Ty& value = _Ty{}) {
                               ...
                               */
                              static constexpr auto compute_stride_and_total(const std::array<std::size_t, Dimension>& dims) noexcept
                              ...
                              /**
                               * @brief Converts a multi-dimensional padded index to a flat offset (without bounds check).
                               *
                               * @param insert Index per dimension (including ghost access).
                               * @return Flat memory offset.
                               */
                              std::intptr_t unsafe_linearize(const std::array<std::intptr_t, Dimension>& insert) const noexcept {
                              ...
                              /**
                               * @brief Converts a multi-dimensional padded index to a flat offset (with bounds check).
                               *
                               * @param insert Index per dimension.
                               * @return Flat memory offset.
                               * @throws std::out_of_range if any index exceeds bounds.
                               */
                              std::intptr_t safe_linearize(const std::array<std::intptr_t, Dimension>& insert) const {
                                        std::intptr_t result = 0;
                                        for (std::size_t i = Dimension; i-- > 0;) {
